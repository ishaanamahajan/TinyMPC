...
extern "C" int main() {
    ...
    // Initial state and lifted representation
    ...
    std::vector<std::array<tinytype,3>> disks = {{ {ox, oy, r_eff} }};

    auto signed_distance = [&](double x, double y, double radius) {
        return std::sqrt((x - static_cast<double>(ox))*(x - static_cast<double>(ox)) +
                         (y - static_cast<double>(oy))*(y - static_cast<double>(oy))) - radius;
    };

    // ---------------- Stage 1: PSD-only solve ----------------
    ...
    // Linear lift-costs on diag(XX) / diag(UU) (copied from tiny_psd_demo.cpp)
    {
    ...
    // Export PSD stage diagnostics
    {
        std::ofstream csv("../psd_tv_pipeline_stage1_psd.csv");
        if (csv.is_open()) {
            csv << "k,x1,x2,x3,x4,u1,u2,signed_dist,iter\n";
            for (int k = 0; k < N; ++k) {
                const Vec& xk = Xpsd[k];
                double sd = signed_distance(xk(0), xk(1), static_cast<double>(r_eff));
            ...
            csv.close();
            std::cout << "[PSD+TV-PIPE] Stage1 rollout -> psd_tv_pipeline_stage1_psd.csv\n";
        } else {
            std::cout << "[EXPORT] Failed to open Stage1 CSV for writing\n";
        }
    }

    // ---------------- Stage 2: TV-only solve tracking PSD path ----------------
    ...
    tiny_enable_base_tangent_avoidance(solver_tv, ox, oy, r_eff, margin);
    ...
    for (int k = 0; k < N; ++k) {
        double sd = signed_distance(Xtv[k](0), Xtv[k](1), static_cast<double>(r_eff));
        if (sd < min_sd_tv) min_sd_tv = sd;
    }
    std::cout << "[PSD+TV-PIPE] Stage2 min signed distance: " << min_sd_tv << "\n";

    // Export combined pipeline data (match psd_tv_combo_trajectory.csv layout)
    {
        std::ofstream csv("../psd_tv_pipeline_stage2_tv.csv");
        if (csv.is_open()) {
            ...
            for (int k = 0; k < N; ++k) {
                ...
                double sd = signed_distance(x1, x2, static_cast<double>(r_eff));
                csv << "," << XX_11 << "," << XX_22 << "," << gap << "," << sd << "," << iters_tv << "\n";
            }
            csv.close();
            std::cout << "[PSD+TV-PIPE] Stage2 rollout -> psd_tv_pipeline_stage2_tv.csv\n";
        } else {
            std::cout << "[EXPORT] Failed to open Stage2 CSV for writing\n";
        }
    }
