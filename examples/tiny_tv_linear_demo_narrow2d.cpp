...
extern "C" int main() {
    ...
    // Initial state: start at (-8, 4) with zero velocity
    Vec x0(NX0);
    x0 << -8.0, -4.0, 0.0, 0.0;
    ...
    for (int j = 0; j < NX0; ++j) {
        for (int i = 0; i < NX0; ++i) {
            x0l(NX0 + j*NX0 + i) = X0(i, j);
        }
    }

    tiny_set_x0(solver, x0l);

    // Reference shaping:
    //  - No explicit far-away target; the origin acts as the natural attractor.
    //  - Add small diag(XX)/diag(UU) refs as before.
    Mat Xref = Mat::Zero(nxL, N);
    Mat Uref = Mat::Zero(nuL, N-1);
    const tinytype q_xx = tinytype(1.0);
    const tinytype r_uu = tinytype(10.0);

    // Small extra linear terms on diag(XX)
    for (int k = 0; k < N; ++k) {
        for (int i = 0; i < NX0; ++i) {
            int idx = NX0 + i*NX0 + i;
    ...
    // Small extra linear terms on diag(UU)
    ...
    std::ofstream csv("../tv_narrow2d_trajectory.csv");
    if (csv.is_open()) {
        ...
        for (int k = 0; k < N; ++k) {
            ...
            // Signed distance to the two obstacles used in this demo:
        ...
        csv.close();
        std::cout << "[EXPORT] tv_narrow2d_trajectory.csv written\n";
    }
